...
# Use the domain interface type so callers depend on the abstraction.
_gmail_client: Optional[IGmailClient] = None


def get_gmail_client() -> IGmailClient:
    """Get the global Gmail client instance (typed to `IGmailClient`)."""
    if _gmail_client is None:
        raise RuntimeError("Gmail client not initialized. Call create_gmail_tools() first.")
    # Narrow to the interface for callers
    return cast(IGmailClient, _gmail_client)
...
class GmailClient:
    ...
    def __init__(
        self, credentials_path: str = "credentials.json", token_path: str = "token.pickle"
    ):
        """
    ...
    def _ensure_service(self) -> None:
        ...
        if self.service is None:
            raise RuntimeError(
                "Gmail service not authenticated. Call create_gmail_tools() or initialize the client."
            )
...
async def list_emails(query: str = "is:unread", max_results: int = 20) -> str:
    ...
    # Get details for each message (domain EmailMessage objects)
    email_summaries = []
    for msg in messages:
        try:
            # msg is an EmailMessage domain object
            details = client.get_message(msg.id)

            email_summaries.append(
                {
                        "id": msg.id,
                    "from": f"{msg.from_address.name} <{msg.from_address.address}>" if getattr(msg.from_address, 'address', None) else "Unknown",
                    "subject": msg.subject or "No subject",
                    "date": getattr(msg.date, 'isoformat', lambda: 'Unknown')(),
                    "snippet": (msg.snippet or "")[:100],
                }
            )
        except Exception as e:
            email_summaries.append(
...
async def delete_emails_by_sender(sender: str, max_delete: int = 500) -> str:
    ...
    result = client.batch_trash_messages(message_ids)

    return f"Successfully moved {result.get('success', 0)} emails from {sender} to trash"
...
async def delete_old_emails(days: int = 30, max_delete: int = 1000) -> str:
    ...
    return f"Successfully moved {result.get('success', 0)} emails older than {days} days to trash"
...
async def search_and_delete(search_term: str, confirm: bool, max_delete: int = 500) -> str:
    ...
    return f"Successfully moved {result.get('success', 0)} emails matching '{search_term}' to trash"
...
def create_gmail_tools(credentials_path: str = "credentials.json") -> List[Tool]:
    ...
    # Cast the concrete client to the domain interface for callers.
    _gmail_client = cast(IGmailClient, GmailClient(credentials_path))
        ...
        for msg in messages:
            try:
                ...
                email_summaries.append(
                    {
                        "id": msg.id,
                        "from": str(msg.from_address) if getattr(msg, 'from_address', None) else "Unknown",
                        "subject": msg.subject or "No subject",
                        "date": getattr(msg.date, 'isoformat', lambda: 'Unknown')(),
                        "snippet": (msg.snippet or "")[:100],
                    }
                ...
                email_summaries.append({"id": getattr(msg, 'id', 'unknown'), "error": str(e)})
